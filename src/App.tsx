...
function App() {
    ...
    const preloadImages = async () => {
      ...
      const loadImage = (src: string) => {
        return new Promise<void>((resolve) => {
          const img = new Image();
          // Fix for GitHub Pages/Vite base path: if path doesn't start with /, pre-pend base if needed, 
          // BUT since we removed /, they are relative. 'src' relative to HTML document base.
          // However, in Vite production, we might generally want to rely on correct relative resolution.
          // Let's just use the src as is, since we fixed storyNodes to be relative "image.png".
          img.src = src;
          img.onload = () => resolve();
          img.onerror = () => {
  ...
  // Loading Screen Overlay (renders on top, then fades out)
  // We remove it from DOM only after animation to keep structure clean
  ...
  // Prevent default touch actions (extra safety for mobile dragging)
  useEffect(() => {
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden bg-black text-white font-sans selection:bg-none cursor-default">
      ...
      <AnimatePresence mode="popLayout">
        <motion.div
          key={currentNodeId} // Force transition on every node change
      ...
      {/* Global Defeat Overlay (Z-8: Above BG, Below Content) */}
      {/* Global Defeat Overlay Removed */}
          ...
          <AnimatePresence mode='wait'>
            {currentNode?.isDefeat ? (
              ...
              <motion.div
                ...
                className="fixed inset-0 z-[100] flex items-center justify-center" // High Z-Index to cover everything
              >
                <motion.div
